...
public class Game extends JPanel implements KeyListener, ActionListener{
	...
	private int levelUpCoundowns = 100;
	private boolean isLvl3Reached = false;
	private boolean isLvl2Reached = false;
	...
	public void paint(Graphics g) {//Paints the objects in each game loop
		...
		if(!isGameStarted) {
			...
			if(level == 2 && levelUpCoundowns>0)
			{
				g.setFont(new Font("TimesRoman", Font.PLAIN, 50));
				g.drawString("Level Up!", 400, 400);
	
				levelUpCoundowns -= 1;
			}
			if(level == 3 && levelUpCoundowns > 0)
			{
				g.setFont(new Font("TimesRoman", Font.PLAIN, 50));
				g.drawString("Level Up!", 400, 400);
				levelUpCoundowns -= 1;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(isGameStarted) {
			...
			if(score > 100 && isLvl2Reached == false) {
				level = 2;
				Cat.setSpeed(4);
				Target.setSpeed(2);
				isLvl2Reached = true;
			}
			if(score > 200 && isLvl3Reached == false)
			{
				level = 3;
				Cat.setSpeed(6);
				Target.setSpeed(4);
